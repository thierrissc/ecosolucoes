  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h2 className="text-white font-bold text-xl">Demandas Semanais</h2>
          <p className="text-slate-400 text-sm">{filtered.length} tarefas</p>
        </div>
        <div className="flex items-center gap-2 bg-brand-navy-light border border-brand-navy-border rounded-xl p-1">
          <button
            onClick={() => setVisualizacao('lista')}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
              visualizacao === 'lista' ? 'bg-brand-green text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
              visualizacao === 'kanban' ? 'bg-brand-green text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
      ...
      <div className="glass-card rounded-2xl p-4 border border-brand-navy-border">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <select
            ...
            className="bg-brand-navy-light border border-brand-navy-border rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none ml-auto"
          >
      ...
      {visualizacao === 'lista' ? (
        <div className="glass-card rounded-2xl border border-brand-navy-border overflow-hidden">
          <div className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-brand-navy-border">
          ...
          <div className="divide-y divide-brand-navy-border">
            {filtered.map((t) => {
              ...
              return (
                <div key={t.id} className="grid grid-cols-12 gap-4 px-5 py-4 hover:bg-white/2 transition-colors items-center">
                  <div className="col-span-5">
                    <p className="text-white text-sm font-medium truncate">{t.nome}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-slate-500 text-xs">{t.responsavel}</span>
                      {setor && (
                        <span className="text-xs px-1.5 py-0.5 rounded" style={{ backgroundColor: setor.cor + '22', color: setor.cor }}>
                          {setor.nome.split(' ')[0]}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="col-span-2">
                    <p className={`text-sm font-medium ${days <= 1 ? 'text-red-400' : days <= 3 ? 'text-orange-400' : 'text-slate-300'}`}>
                      {formatDate(t.prazo)}
                    </p>
                    <p className="text-slate-500 text-xs">{days <= 0 ? 'Vencido!' : `${days}d restantes`}</p>
                  </div>
                  <div className="col-span-2">
                  ...
                  <div className="col-span-3 flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="bg-brand-navy-light border border-brand-navy-border rounded-lg px-2 py-1 text-xs text-white focus:outline-none ml-auto"
                    >
                      <option value="nao_iniciada">Não Iniciada</option>
                      <option value="em_andamento">Em Andamento</option>
                      <option value="concluida">Concluída</option>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {kanbanColunas.map((col) => {
            ...
            return (
              <div key={col.status} className={`glass-card rounded-2xl border border-brand-navy-border border-t-4 ${col.color} kanban-column p-4`}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-white font-semibold text-sm">{col.label}</h3>
                  <span className="bg-brand-navy-border text-slate-300 text-xs font-bold px-2 py-0.5 rounded-full">
                    {colTarefas.length}
                  </span>
                </div>
